      ...
      if( result_with_node ) {
        if( result_with_node.initNode ) {
          if( typeof result_with_node.initNode !== 'function' ) {
            console.error('initNode should be a function', result_with_node.initNode );
          }
...
RenderApp.prototype.directive = function (directive, initNode, withNode) {
  ...
  this.with_node_pipe.push(function (node) {
    var attr_key = node.attrs && _.find( Object.keys(node.attrs), matchAttr);

    if( attr_key ) return _.extend( withNode && withNode(node, attr_key) || {}, {
