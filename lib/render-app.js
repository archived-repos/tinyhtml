...
RenderApp.prototype.component = function (tag_name, initNode, with_node) {
  ...
  this.with_node_pipe.push(function (node) {
    if( node.$ === tag_name ) return with_node ? _.extend( with_node, { initNode: initNode }) : { initNode: initNode };
  });
...
RenderApp.prototype.directive = function (directive, initNode, withNode) {
  ...
  this.with_node_pipe.push(function (node) {
    var attr = node.attrs && _.find( Object.keys(node.attrs), matchAttr);

    if( attr ) return _.extend( withNode && withNode(node, attr) || {}, {
