...
var _ = require('./utils'),
    ...
    createConText = require('./con-text');
...
function createApp(options) {
  ...
  data_app.withNode(function (node) {
    if( typeof node.text === 'string' ) return {
      initNode: function (el) {
        ...
        if( el.parentElement ) el.parentElement.insertBefore( document.createComment(' text: ' + node.text + ' '), el );
  ...
  data_app.render = function (_parent, _nodes, render_options) {
    ...
    var children = app.render.apply(this_app, arguments);
